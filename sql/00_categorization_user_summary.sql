...
user_edit_days AS (
  SELECT DISTINCT user_id, edit_date::DATE AS edit_day
  FROM user_edits
),
ranked_days AS (
  SELECT
    user_id,
    edit_day,
    ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY edit_day) AS rn
  FROM user_edit_days
),
...
burstiness AS (
  WITH user_edit_days AS (
    -- Nur einzigartige Edit-Tage pro Nutzer
    SELECT DISTINCT
      user_id,
  ...
  edit_diffs AS (
    -- Abstände in Tagen zwischen zwei Edit-Tagen
    SELECT
      user_id,
      day,
  ...
  stats AS (
    -- Ø-Abstand (mu) und Standardabweichung (sigma) pro Nutzer
    SELECT
      user_id,
      AVG(diff_days) AS mu,
      STDDEV(diff_days) AS sigma
    FROM edit_diffs
  ...
  SELECT
    user_id,
    sigma AS stddev_days,  -- deine alte Variante
    mu AS mean_days,       -- Durchschnittsabstand
    (sigma - mu) * 1.0 / NULLIF(sigma + mu, 0) AS burstiness_score
  FROM stats
),
...
weekly_stats AS (
  SELECT
    ...
    DATEDIFF('week', f.first_edit, f.full_last_edit) + 1 AS total_weeks,
    COUNT(*) * 1.0 / NULLIF(DATEDIFF('week', f.first_edit, f.full_last_edit) + 1, 0) AS active_week_ratio
  FROM weekly w
  JOIN first_edit_per_user f ON w.user_id = f.user_id
  GROUP BY w.user_id, f.first_edit, f.full_last_edit
),
early_windows AS (
  SELECT
    r.user_id,
...
SELECT
  ...
  ted.true_edit_days * 1.0 /active_duration AS activity_ratio_true,
  ...
  er.frontload_ratio,
  er.early_days_ratio,
  ted.true_edit_days * 1.0 / NULLIF(CASE WHEN ted.true_edit_days > 0 THEN DATE_DIFF('day', fepu.first_edit, fepu.full_last_edit)+1 ELSE 0 END, 0) AS activity_density,
  us.total_edits * 1.0 / NULLIF(DATE_DIFF('day', fepu.first_edit, fepu.full_last_edit)+1, 0) AS edits_per_span_day,
...
LEFT JOIN span_total        sp  ON us.user_id = sp.user_id
